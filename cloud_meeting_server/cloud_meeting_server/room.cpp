...
static volatile int maxfd;	//volatile直接存取原始内存地址.每次需要存储或读取这个变量的时候直接从变量内存地址中读取数据。和const是相对的
...
typedef struct pool
{
	...
	pool()
	{
		memset(status, 0, sizeof(status));	// void *memset(void *str, int c, size_t n) 复制字符 c（一个无符号字符）到参数 str 所指向的字符串的前 n 个字符。
		owner = 0;
		FD_ZERO(&fdset);
			...
			if (FD_ISSET(i, &rset))
			{
				...
				else if (ret  == 11)
				{
					if (head[0] == '$')
					{
						//solve datatype
						MSG_TYPE msgtype;
						memcpy(&msgtype, head + 1, 2);	// void *memcpy(void *str1, const void *str2, size_t n) 从存储区 str2 复制 n 个字节到存储区 str1
						msgtype = (MSG_TYPE)ntohs(msgtype);	//ntohs将一个16位数由网络字节顺序转换为主机字节顺序，简单的说就是把一个16位数高低位互换。
						...
						msg.len = ntohl(msglen);	//将一个32位数由网络字节顺序转换为主机字节顺序。
